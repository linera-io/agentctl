...
| **Ghostty** | Yes (macOS) | Yes | Yes | Yes | Native AppleScript API, no Kitty-style remote control setup |
...
| **iTerm2** | Yes (macOS) | Yes | Yes | Yes | AppleScript + System Events |
| **Terminal.app** | Yes (macOS) | Yes | Yes | Yes | AppleScript + System Events |
...
- **Ghostty, iTerm2, Terminal.app**: Visible launch (`--new` / `n`) opens a new window running `claude` via AppleScript, and works only on a macOS host.
