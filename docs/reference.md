...
| `--restore-sessions` | Restore this host's own Claude sessions — e.g. after a Ghostty restart-to-update — spawning one window per session that was live, each running `claude --resume <id>` in its recorded directory. No argument; pair with `--dry-run` to preview |
